...
public class ${Table}Controller {
    ...
    public PageResult<${Table}> findPage(@RequestBody(required = false) <#if swagger==true>@ApiParam(name = "${Table}对象",value = "传入JSON数据",required = false)</#if> ${Table} ${table}, @PathVariable  int page, @PathVariable  int size){
        //调用${Table}Service实现分页条件查询${Table}
        PageInfo<${Table}> pageInfo = ${table}Service.findPage(${table}, page, size);
        return new PageResult<${Table}>(pageInfo.getTotal(),pageInfo.getList());
    }
    ...
    public PageResult<${Table}> findPage(@PathVariable  int page, @PathVariable  int size){
